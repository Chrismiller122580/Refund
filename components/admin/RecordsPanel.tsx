  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900">Search all records</h2>
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 px-6 py-4">
          ...
          <p className="mt-1 text-sm text-slate-600">{records.length} shown</p>
        </div>
        {loading ? (
          <p className="px-6 py-8 text-sm text-slate-500">Loading records…</p>
        ) : records.length === 0 ? (
          <p className="px-6 py-8 text-sm text-slate-500">No records found.</p>
        ) : (
                ...
                {records.map((record) => (
                  <tr key={record.id} className="hover:bg-slate-50/80">
                    ...
                    <td className="px-6 py-4">
                      <span className="inline-flex rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium uppercase text-slate-700">
                        {record.type}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-slate-600">{record.userEmail ?? '—'}</td>
                    <td className="px-6 py-4 text-slate-600">
                      {record.inputs.startDate} → {record.inputs.endDate}
                    </td>
                    <td className="px-6 py-4 text-slate-900">{customerTotal(record) ?? '—'}</td>
                    <td className="px-6 py-4 text-slate-500">{formatSavedAt(record.savedAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
